                ...
                <div class="card-body">
                    ...
                    <p class="text-center small">Enter your email to reset your password</p>
             		...
             		<x-auth-session-status class="mb-4" :status="session('status')" />
                  	<form class="row g-3 needs-validation login_from" method="POST" action="{{ route('password.store') }}" id="reset_password_form">
                       @csrf
                        <input type="hidden" name="token" value="{{ $request->route('token') }}">
                    <div class="col-12">
                      <label for="email" class="form-label">Email</label>
                      <div class="input-group has-validation email-input">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="email" class="form-control email-input" name="email" id="email" placeholder="Enter User Email" value="{{old('email', $request->email)}}"  autofocus required>
